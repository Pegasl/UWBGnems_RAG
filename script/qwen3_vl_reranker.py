...
class Qwen3VLReranker():
    ...
    def tokenize(self, pairs: List[Dict], **kwargs) -> Dict:
        ...
        # Truncate input IDs and mm_token_type_ids while preserving special tokens
        for i, ele in enumerate(inputs['input_ids']):
            orig_len = len(ele)
            if orig_len > max_length:
                special_tokens_set = set(self.processor.tokenizer.all_special_ids)
                
                num_special = sum(1 for token in ele if token in special_tokens_set)
                num_non_special_to_keep = max_length - num_special
                ...
                has_mm_type = 'mm_token_type_ids' in inputs
                mm_type = inputs['mm_token_type_ids'][i] if has_mm_type else None
                
                # Protect the last 5 tokens (typically prompt/generation tokens)
                end_idx = orig_len - 5
                
                for j in range(end_idx):
                    token = ele[j]
                    if token in special_tokens_set:
                        final_input_ids.append(token)
                        if has_mm_type: final_mm_type_ids.append(mm_type[j])
                    elif non_special_kept_count < num_non_special_to_keep:
                        final_input_ids.append(token)
                        if has_mm_type: final_mm_type_ids.append(mm_type[j])
                        non_special_kept_count += 1
                        
                # Always append the last 5 tokens
                final_input_ids.extend(ele[-5:])
                inputs['input_ids'][i] = final_input_ids
                
                if has_mm_type:
                    final_mm_type_ids.extend(mm_type[-5:])
                    inputs['mm_token_type_ids'][i] = final_mm_type_ids
            
        # Apply padding for all sequence-level features
        pad_dict = {'input_ids': inputs['input_ids']}
        if 'mm_token_type_ids' in inputs:
            pad_dict['mm_token_type_ids'] = inputs['mm_token_type_ids']
        ...
        # Update inputs with the padded tensors
        for key in temp_inputs:
            inputs[key] = temp_inputs[key]
            
        # Ensure image/video inputs are also converted to tensors if they aren't already
        for key in ['pixel_values', 'pixel_values_videos', 'image_grid_thw', 'video_grid_thw']:
            if key in inputs and isinstance(inputs[key], list):
                if not isinstance(inputs[key][0], torch.Tensor):
                    inputs[key] = torch.tensor(inputs[key])
                else:
                    inputs[key] = torch.cat(inputs[key], dim=0) if len(inputs[key]) > 0 else inputs[key]
